...
solution = [[0] for _ in range(P)]  # solution that the algorithm will find
...
def aco(p, ant, fitness):
    ...
    for iteration in range(maxIteration):  # run the iterations
        ...
        if fitness[currentAnt] < fitness[bestAnt]:  # if the fitness of currently chosen ant is lesser than the old best fitness
            ...
            if oldBestFitness != 0 and abs(fitness[bestAnt] - oldBestFitness) / abs(oldBestFitness) <= 1 / 1e37:  # condition to break the loop
                break
            for a in range(len(ant)):  # for each ant in population
    ...
    print(f"Total number of iterations to obtain the solution for patient {p + 1} by ACO is {iteration + 1}")
    totalIteration += iteration + 1
    return ant[bestAnt]  # return the best ant


# function to export the solution obtained, start and end time taken by ACO algorithm to solve data offloading objective
def export_ACO_Data_Metrics():
    return [solution, startTime, endTime]


# helper function to run ACO algorithm for data offloading objective
def ACO_DataOffloading():
    global startTime, endTime, solution, totalIteration
    startTime = time.time()  # get the start time of the algorithm
    pat = getPatientList()  # get the list of patients sorted in decreasing order of their criticality
    for p in range(P):  # for each patient
        ant = initializeDataWhalePopulation()  # get the ant population, which is same as the whale population in NGS-WOA algorithm for data offloading
        solution[pat[p]] = aco(pat[p], ant, calculateDataWhaleFitness(pat[p], ant))  # get the solution of ACO algorithm
    endTime = time.time()  # get the end time of the algorithm
    print("Time taken to obtain data offloading solution by ACO: ", endTime - startTime)
    print("Average number of iterations taken: ", totalIteration / P)
    totalIteration = 0
    print("The data offloading decision matrix obtained by ACO is:")
